...
**Date:** 2026-05-25  
**Scope:** Full 3-file audit — `src/goldfishh/init.py`, `CLAUDE.md`, `AGENTS.md`  
...
| 4 | Layer 1/2 boundary not visible in rendered context | `CLAUDE.md` | Agent can't tell where goldfishh coordination ends and tool blocks begin |
...
Replace the current `_CLAUDE_MD_BLOCK` string in `src/goldfishh/init.py` with:
...
**After updating `init.py`:** Re-apply to `CLAUDE.md` by manual replacement — edit the content between the goldfishh sentinel (`## Agent Knowledge Tools (managed by goldfishh)`) and the `<!-- layer 1: tool blocks -->` comment. Manual edit is preferred here because the same pass fixes the orphaned gitnexus block and removes the hook routing section; doing all three edits together avoids multiple file rewrites.
...
**Current problem:** "What goldfishh contributes" and "Further reading" sit inside the `<!-- layer 2: goldfishh -->` block, but they're project-level facts — Layer 3 material.
...
## What goldfishh contributes      ← MOVED HERE (was in layer 2)
...
## Superpowers workflow (required for all contributors)   ← goldfishh coordination only
...
**Principle:** Layer 3 = what this project is and its constraints. Layer 2 = how goldfishh coordinates the agent's workflow. Layer 1 = tool-native auto-maintained blocks.
...
| File | Change |
|------|--------|
| `src/goldfishh/init.py` | Replace `_CLAUDE_MD_BLOCK` string with new version |
| `CLAUDE.md` | Delete hook routing section; replace goldfishh block with new content; remove orphaned gitnexus block |
| `AGENTS.md` | Move "What goldfishh contributes" + "Further reading" above layer 2 marker |
...
- `_CLAUDE_MD_BLOCK` in `init.py` matches what appears in `CLAUDE.md`'s goldfishh block
...
- `AGENTS.md` Layer 2 contains only goldfishh workflow coordination (no project facts)
