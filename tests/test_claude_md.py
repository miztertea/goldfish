...
def test_append_claude_md_block_adds_block(tmp_path):
    ...
    append_claude_md_block(claude_md, GOLDFISHH_SENTINEL + "\nsome content\n")
    assert GOLDFISHH_SENTINEL in claude_md.read_text()


def test_append_claude_md_block_is_idempotent(tmp_path):
    claude_md = tmp_path / "CLAUDE.md"
    claude_md.write_text("# Existing content\n")
    block = GOLDFISHH_SENTINEL + "\nsome content\n"
    append_claude_md_block(claude_md, block)
    append_claude_md_block(claude_md, block)
    assert claude_md.read_text().count(GOLDFISHH_SENTINEL) == 1
