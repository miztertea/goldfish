...
from goldfish.init import _run, check_dependency, run
...
def test_run_returns_result_on_success():
    ...
    with patch("goldfish.init.subprocess.run", return_value=ok):
...
def test_run_returns_none_on_file_not_found_non_windows():
    with (
        patch("goldfish.init.subprocess.run", side_effect=FileNotFoundError),
...
def test_run_returns_none_on_os_error_non_windows():
    with (
        patch("goldfish.init.subprocess.run", side_effect=OSError),
        patch("sys.platform", "linux"),
    ):
        result = _run(["missing-command"])
    assert result is None
...
def test_run_passes_kwargs_through():
    ok = MagicMock(returncode=0)
    with patch("goldfish.init.subprocess.run", return_value=ok) as mock_run:
...
def test_check_dependency_returns_false_on_file_not_found():
    with patch("goldfish.init.subprocess.run", side_effect=FileNotFoundError):
        assert check_dependency("nonexistent-tool") is False


def test_check_dependency_returns_false_on_nonzero_returncode():
    with patch("goldfish.init.subprocess.run", return_value=MagicMock(returncode=1)):
        assert check_dependency("broken-tool") is False


def test_check_dependency_returns_true_on_zero_returncode():
    with patch("goldfish.init.subprocess.run", return_value=MagicMock(returncode=0)):
